...
export default function RotatePDFPage() {
  ...
  const handleRotate = async () => {
    ...
    setProcessStatus(`Rotating PDF ${rotation} degrees...`)
    try {
      await new Promise(resolve => setTimeout(resolve, 2000))
      setProcessStatus('✅ Rotation completed. Feature coming soon!')
    } catch (error: any) {
  ...
  return (
    <div className="min-h-screen bg-gray-900">
      <Navbar />
      <section className="bg-gradient-to-br from-gray-800 via-gray-700 to-teal-900 py-16">
        <div className="max-w-6xl mx-auto px-3 sm:px-6 lg:px-8">
          <div className="text-center">
            <h1 className="text-4xl md:text-5xl font-bold text-white mb-4">Rotate PDF</h1>
            <p className="text-lg md:text-xl text-gray-300 max-w-2xl mx-auto">Fix document orientation by rotating PDF pages.</p>
          </div>
        </div>
      </section>
      <section className="py-16 bg-gray-800">
        <div className="max-w-4xl mx-auto px-3 sm:px-6 lg:px-8">
          <div className="bg-white rounded-xl p-4 md:p-8 mb-8 shadow-lg">
            <div className={`border-2 border-dashed rounded-lg p-4 md:p-8 text-center transition-colors cursor-pointer ${isDragOver ? 'border-teal-500 bg-teal-50' : 'border-gray-300 bg-gray-50 hover:border-teal-400'}`}
              onDragOver={handleDragOver} onDragLeave={handleDragLeave} onDrop={handleDrop} onClick={handleFileSelect}>
              <input ref={fileInputRef} type="file" accept=".pdf" onChange={handleFileChange} className="hidden" />
              <FiUpload className={`h-10 w-10 md:h-12 md:w-12 mx-auto mb-3 ${isDragOver ? 'text-teal-500' : 'text-gray-400'}`} />
              {!selectedFile ? (
                <>
                  <h3 className="text-base md:text-lg font-semibold text-gray-700 mb-2"><span className="text-teal-600 underline">Drop your PDF here or browse</span></h3>
                  <p className="text-xs md:text-sm text-gray-500">Supports: .pdf files only</p>
                </>
              ) : (
                <>
                  <h3 className="text-base md:text-lg font-semibold text-gray-700 mb-2"><span className="text-teal-600 underline">Add more files or drop here</span></h3>
          ...
          {selectedFile && (
            <>
              <div className="bg-white rounded-xl p-4 md:p-8 shadow-lg mb-8">
                <h3 className="text-lg font-bold text-gray-800 mb-4">Rotation Options</h3>
                <div className="space-y-4">
                  <div><label className="block text-sm font-medium text-gray-700 mb-2">Rotation Angle</label>
                    <div className="flex gap-4">
                      <button onClick={() => setRotation(90)} className={`px-6 py-3 rounded-lg ${rotation === 90 ? 'bg-teal-500 text-white' : 'bg-gray-100 text-gray-700'}`}>90°</button>
                      <button onClick={() => setRotation(180)} className={`px-6 py-3 rounded-lg ${rotation === 180 ? 'bg-teal-500 text-white' : 'bg-gray-100 text-gray-700'}`}>180°</button>
                      <button onClick={() => setRotation(270)} className={`px-6 py-3 rounded-lg ${rotation === 270 ? 'bg-teal-500 text-white' : 'bg-gray-100 text-gray-700'}`}>270°</button>
                    </div>
                  </div>
                </div>
              </div>
              <div className="bg-white rounded-xl p-4 md:p-8 shadow-lg">
                <button onClick={handleRotate} disabled={isProcessing} className="w-full bg-gradient-to-r from-teal-500 to-teal-600 hover:from-teal-600 hover:to-teal-700 disabled:from-gray-400 disabled:to-gray-500 text-white font-semibold py-3 px-6 rounded-lg transition-all duration-200 shadow-lg hover:shadow-xl flex items-center justify-center disabled:cursor-not-allowed">
                  {isProcessing ? (<> <div className="animate-spin rounded-full h-5 w-5 border-b-2 border-white mr-2"></div> Rotating... </>) : (<> <FiRotateCw className="h-5 w-5 mr-2" /> Rotate PDF {rotation}° </>)}
                </button>
                {processStatus && (
