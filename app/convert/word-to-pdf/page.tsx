...
import { FiUpload, FiFile, FiDownload, FiShield, FiX, FiArrowLeft } from 'react-icons/fi'
...
export default function WordToPDFPage() {
  ...
  const handleFileChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file && (file.type.includes('word') || file.name.endsWith('.doc') || file.name.endsWith('.docx'))) {
  ...
  const handleConvert = async () => {
    ...
    setProcessStatus('Converting Word to PDF...')
    try {
      await new Promise(resolve => setTimeout(resolve, 2000))
      setProcessStatus('✅ Conversion initiated. Feature coming soon!')
    } catch (error: any) {
  ...
  return (
    <div className="min-h-screen bg-gray-900">
      <Navbar />
      <section className="bg-gradient-to-br from-gray-800 via-gray-700 to-purple-900 py-16">
        <div className="max-w-6xl mx-auto px-3 sm:px-6 lg:px-8">
          <div className="text-center">
            <h1 className="text-4xl md:text-5xl font-bold text-white mb-4">Word to PDF</h1>
            <p className="text-lg md:text-xl text-gray-300 max-w-2xl mx-auto">Convert Word documents to PDF format with advanced formatting options.</p>
          </div>
        </div>
      </section>
      <section className="py-16 bg-gray-800">
        <div className="max-w-4xl mx-auto px-3 sm:px-6 lg:px-8">
          <div className="bg-white rounded-xl p-4 md:p-8 mb-8 shadow-lg">
            <div className={`border-2 border-dashed rounded-lg p-4 md:p-8 text-center transition-colors cursor-pointer ${isDragOver ? 'border-purple-500 bg-purple-50' : 'border-gray-300 bg-gray-50 hover:border-purple-400'}`}
              ...
              <FiUpload className={`h-10 w-10 md:h-12 md:w-12 mx-auto mb-3 ${isDragOver ? 'text-purple-500' : 'text-gray-400'}`} />
              {!selectedFile ? (
                <>
                  <h3 className="text-base md:text-lg font-semibold text-gray-700 mb-2"><span className="text-purple-600 underline">Drop your Word file here or browse</span></h3>
                  <p className="text-xs md:text-sm text-gray-500">Supports: .doc, .docx files</p>
                </>
              ) : (
                <>
                  <h3 className="text-base md:text-lg font-semibold text-gray-700 mb-2"><span className="text-purple-600 underline">Add more files or drop here</span></h3>
            ...
            {selectedFile && (
              <div className="mt-6 bg-gray-50 rounded-lg p-4">
                <div className="flex items-center justify-between">
                  <div className="flex items-center flex-1"><FiFile className="h-5 w-5 text-blue-500 mr-3" />
          ...
          {selectedFile && (
            <div className="bg-white rounded-xl p-4 md:p-8 shadow-lg">
              <button onClick={handleConvert} disabled={isProcessing} className="w-full bg-gradient-to-r from-purple-500 to-purple-600 hover:from-purple-600 hover:to-purple-700 disabled:from-gray-400 disabled:to-gray-500 text-white font-semibold py-3 px-6 rounded-lg transition-all duration-200 shadow-lg hover:shadow-xl flex items-center justify-center disabled:cursor-not-allowed">
                {isProcessing ? (<> <div className="animate-spin rounded-full h-5 w-5 border-b-2 border-white mr-2"></div> Converting... </>) : (<> <FiDownload className="h-5 w-5 mr-2" /> Convert to PDF </>)}
              </button>
              {processStatus && (
