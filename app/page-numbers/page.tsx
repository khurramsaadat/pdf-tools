...
export default function PageNumbersPage() {
  ...
  const handleAddPageNumbers = async () => {
    ...
    setProcessStatus('Adding page numbers...')
    try {
      await new Promise(resolve => setTimeout(resolve, 2000))
      setProcessStatus('✅ Page numbers added. Feature coming soon!')
    } catch (error: any) {
  ...
  return (
    <div className="min-h-screen bg-gray-900">
      <Navbar />
      <section className="bg-gradient-to-br from-gray-800 via-gray-700 to-emerald-900 py-16">
        <div className="max-w-6xl mx-auto px-3 sm:px-6 lg:px-8">
          <div className="text-center">
            <h1 className="text-4xl md:text-5xl font-bold text-white mb-4">Add Page Numbers</h1>
            <p className="text-lg md:text-xl text-gray-300 max-w-2xl mx-auto">Add professional page numbering to your PDF documents.</p>
          </div>
        </div>
      </section>
      <section className="py-16 bg-gray-800">
        <div className="max-w-4xl mx-auto px-3 sm:px-6 lg:px-8">
          <div className="bg-white rounded-xl p-4 md:p-8 mb-8 shadow-lg">
            <div className={`border-2 border-dashed rounded-lg p-4 md:p-8 text-center transition-colors cursor-pointer ${isDragOver ? 'border-emerald-500 bg-emerald-50' : 'border-gray-300 bg-gray-50 hover:border-emerald-400'}`}
              onDragOver={handleDragOver} onDragLeave={handleDragLeave} onDrop={handleDrop} onClick={handleFileSelect}>
              <input ref={fileInputRef} type="file" accept=".pdf" onChange={handleFileChange} className="hidden" />
              <FiUpload className={`h-10 w-10 md:h-12 md:w-12 mx-auto mb-3 ${isDragOver ? 'text-emerald-500' : 'text-gray-400'}`} />
              {!selectedFile ? (
                <>
                  <h3 className="text-base md:text-lg font-semibold text-gray-700 mb-2"><span className="text-emerald-600 underline">Drop your PDF here or browse</span></h3>
                  <p className="text-xs md:text-sm text-gray-500">Supports: .pdf files only</p>
                </>
              ) : (
                <>
                  <h3 className="text-base md:text-lg font-semibold text-gray-700 mb-2"><span className="text-emerald-600 underline">Add more files or drop here</span></h3>
          ...
          {selectedFile && (
            <>
              <div className="bg-white rounded-xl p-4 md:p-8 shadow-lg mb-8">
                <h3 className="text-lg font-bold text-gray-800 mb-4">Page Number Settings</h3>
                <div className="space-y-4">
                  <div><label className="block text-sm font-medium text-gray-700 mb-2">Position</label>
                    <div className="flex gap-4">
                      <button onClick={() => setPosition('top')} className={`px-6 py-3 rounded-lg ${position === 'top' ? 'bg-emerald-500 text-white' : 'bg-gray-100 text-gray-700'}`}>Top</button>
                      <button onClick={() => setPosition('bottom')} className={`px-6 py-3 rounded-lg ${position === 'bottom' ? 'bg-emerald-500 text-white' : 'bg-gray-100 text-gray-700'}`}>Bottom</button>
                    </div>
                  </div>
                </div>
              </div>
              <div className="bg-white rounded-xl p-4 md:p-8 shadow-lg">
                <button onClick={handleAddPageNumbers} disabled={isProcessing} className="w-full bg-gradient-to-r from-emerald-500 to-emerald-600 hover:from-emerald-600 hover:to-emerald-700 disabled:from-gray-400 disabled:to-gray-500 text-white font-semibold py-3 px-6 rounded-lg transition-all duration-200 shadow-lg hover:shadow-xl flex items-center justify-center disabled:cursor-not-allowed">
                  {isProcessing ? (<> <div className="animate-spin rounded-full h-5 w-5 border-b-2 border-white mr-2"></div> Processing... </>) : (<> <FiHash className="h-5 w-5 mr-2" /> Add Page Numbers </>)}
                </button>
                {processStatus && (
